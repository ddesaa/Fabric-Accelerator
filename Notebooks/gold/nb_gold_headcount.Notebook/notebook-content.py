...
source_query = """select distinct 
    (ca.YearMonth*100)+1 DateID, 
    ed.EmployeeID, 
    ed.DepartmentID,
    ShiftID,
    sp.SalesPersonID
 from 
    Silver.Calendar ca 
cross join 
    Bronze.employeedepartment ed on ca.date between ed.StartDate and coalesce(ed.EndDate,Current_date())
left join 
    Silver.salesperson sp on sp.SalesPersonID=ed.EmployeeID
    """
...
upsertDelta(df,"Headcount","DateID|EmployeeID|DepartmentID|ShiftID|SalesPersonID")
